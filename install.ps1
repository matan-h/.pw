...
foreach ($command in ( "fzf", "lsd", "winfetch", "ntop", "gsudo", "duf")) { if ( -Not (Get-Command $command -errorAction SilentlyContinue)) { scoop install $command } }

foreach ($mod in ("posh-git", "PSFzf", "PSReadline", "PSScriptTools","PSWindowsUpdate")) { Install-Module $mod -Confirm:$False -Force }
